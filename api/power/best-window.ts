...
import { isValidArea, getDKDateStr, fetchDayPrices, calcBestWindow } from '../_lib/power';

export default async function handler(req: VercelRequest, res: VercelResponse) {
  const area = (req.query.area as string) ?? 'DK1';
  const durationHours = parseInt((req.query.durationHours as string) ?? '3', 10);
  ...
  try {
    ...
    const bestWindow = calcBestWindow(prices, durationHours);

    res.setHeader('Cache-Control', 's-maxage=1800, stale-while-revalidate');
    return res.status(200).json(bestWindow);
  } catch (err: unknown) {
